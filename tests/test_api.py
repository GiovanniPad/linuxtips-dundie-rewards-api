...
# Dicionários para usar de apoio para validar as respostas.
USER_RESPONSE_KEYS = {"name", "username", "dept", "avatar", "bio", "currency"}
USER_RESPONSE_WITH_BALANCE_KEYS = USER_RESPONSE_KEYS | {"balance"}


# As fixtures são utilizadas diretamente como injeção de dependência nas funções de teste.
...
def test_user_list(
    ...
    """
    users = api_client_admin.get("/user/").json()
...
# Testar se a rota /user/{username} está exibindo os dados do usuário.
@pytest.mark.order(2)
def test_user_detail(api_client_user1):
...
# Teste para validar se o admin consegue alterar os dados de um usuário qualquer.
@pytest.mark.order(3)
...
# Teste para validar se o usuário consegue alterar seus próprios dados.
@pytest.mark.order(3)
...
# Teste para validar se vai gerar um erro quando o usuário normal tentar atualizar os dados
# de um outro usuário.
@pytest.mark.order(3)
def test_fail_update_user_profile_by_other_user(api_client_user2):
    ...
    response = api_client_user2.patch("/user/user1/", json={"bio": "just some bio"})
    assert response.status_code == 403


# Teste para validar se o admin consegue adicionar uma transação para todos os usuários.
@pytest.mark.order(4)
...
# Teste para validar uma transferência de 20 pontos de um usuário a outro.
@pytest.mark.order(5)
...
# Teste para validar se o admin consegue listar os dados dos usuários junto
# do saldo de pontos.
@pytest.mark.order(6)
...
# Teste para validar de o admin consegue listar todas as transações feitas.
@pytest.mark.order(6)
...
# Teste para validar se um usuário normal consegue listar apenas as suas transações.
@pytest.mark.order(6)
def test_regular_user_can_see_only_own_transactions(api_client_user3):
...
# Teste para validar se o usuário admin consegue listar todas as transações utilizando o
